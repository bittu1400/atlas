...
| D129 | A Timing Plan's duration is derived, never supplied (T-20, R-04) | **`TimingPlan.total_duration_seconds` is a computed field taken from the last beat's `end_time_seconds`; the `default=60.0` is gone and the kwarg is dropped at all three construction sites.** The task offered two shapes — make the field required, or compute it — and computing it is the only one that closes the defect. A required field still lets a caller pass `60.0` over 3.5 seconds of beats, which is the same lie with an extra keystroke; a derived value cannot disagree with the timeline it is derived from. The persisted column and its `total_duration_check` constraint are unchanged and now written from the computed value, so no migration is needed. `tests/__init__.py` was added so the new test could reuse `InMemoryExecutionRepository` rather than copy it — without it, `tests/unit/` was a package while `tests/` was not, and mypy resolved the same file under two module names. | — |
| D130 | An unpushed fix is not a fix (T-00) | **CI stayed red for a day against code that was already correct on the laptop.** The `alembic/env.py` change that reads `ATLAS_DATABASE_SYNC_URL` was committed on 2026-09-03 and left unpushed by the operator-decision note in `STATUS.md` §1, so every CI run re-executed the revision without it and failed on `fe_sendauth: no password supplied`. The debugging cost was spent re-deriving a root cause that had already been found and fixed. Recorded because the failure mode is not technical and will recur: **CI tests `origin`, not the working tree**, and a session that ends with commits ahead of `origin` has left its own verification undone. | — |
| D131 | Playwright's browsers are installed from the web workspace | **`ci.yml` runs `pnpm --filter @atlas/web exec playwright install`, not `pnpm exec`.** `@playwright/test` is a devDependency of `apps/web`, so at the repository root the binary does not resolve and the step died with `ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL`. Rejected the alternative of hoisting `@playwright/test` into the root `package.json`: it adds a dependency to a workspace that does not use it in order to fix a path, and the filtered form already matches the idiom the root `test` script uses. | — |
| D132 | `main` requires the `test` check, and admins can still override (T-11) | **Branch protection set via `gh api`: `required_status_checks.contexts = ["test"]`, force pushes and deletions disabled, `enforce_admins` left false.** `strict` is false too — a PR need not be rebased on current `main` to merge, which was rejected because a single-operator repository gets stale-branch friction with no cross-PR conflicts to protect against. `enforce_admins` false is the load-bearing caveat: **"blocking" means the merge button is blocked, not that the gate is unbypassable.** Made explicit because a register that says "CI is blocking" and a repository where the owner can click through are two different safety claims, and this repository has already been burned once by a status claim that was read as stronger than it was (**R7**). T-11's *Done when* — a deliberate lint error failing CI — was **not** literally exercised: two genuine failures the same day (the migration step, the Playwright step) already demonstrated the build goes red on non-zero exit, and pushing a knowingly broken commit for no new information would put it in the branch history permanently. | — |
| D133 | ffmpeg is installed in CI rather than the test being left to skip (T-00 follow-on) | **`ci.yml` runs `apt-get install -y ffmpeg` before pytest.** `test_production_adapters.py:148` is `skipif(shutil.which("ffmpeg") is None)`, and the runner image has no ffmpeg, so the only test that shells out to real ffmpeg passed locally and skipped in CI — green CI did not cover the renderer path at all. Rejected leaving it as a documented note: a skip that is invisible in a green run is exactly the shape of hazard **R10** names, a check that exists without running. Costs ~15 s per run, which is worth it for the one adapter test that touches a real subprocess. | — |
| D134 | V-14 is recorded, not fixed | **The Timing Plan accumulates where ADR-0006 §2 promises fitting; the fix is task T-61, not this session's commit.** Found by reading ADR-0006 against `_compute_timing_plan` while closing T-20 — closing R-04 made the duration honest, which is what made the absence of any mechanism to *control* that duration visible. Fixing it is either a solver plus a repair path routing back to the Script stage, or an ADR superseding ADR-0006 to say Atlas accumulates and the prompt must constrain the sum. Both are behaviour changes with a design decision inside them, and **D106** and **D125** already settled that a documentation session does not quietly change behaviour. Recorded in four registers plus a task with a *Done when*, and sequenced **before T-34**, because a 36–81 s spread meeting a 58–62 s gate on a 20-request daily budget is the precise scarcity that produced the 2026-08-29 fabrication incident. | — |
| D135 | V-15 and V-16 are worked before T-61 | **The Timing Plan task drops to position 3 in audit §15.9; T-62 and T-63 take positions 1 and 2.** T-61 is a real P2 and its sequencing argument — a 36–81 s spread meeting a 58–62 s gate on a scarce budget (**D134**) — still stands. But it is a defect *inside a Run*, and as of 2026-09-05 no operator can start a Run at all: nothing outside `tests/` calls `save_topic`, so `POST /runs` fails its foreign key for every input (**V-15**), and it fails as an untyped 500 (**V-16**). Fixing the timing of a pipeline nobody can enter is optimising a path that is unreachable. Both new defects were found by operating the system as an operator rather than by working the register, which is now the fourth consecutive session where that produced the session's most serious finding. | — |
