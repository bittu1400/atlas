...
export const KineticText: React.FC<KineticTextProps> = ({
  ...
  const opacity = interpolate(
    frame,
    [0, 10, beat.durationFrames - 10, beat.durationFrames],
    [0, 1, 1, 0],
  ...
  const wordsToReveal = Math.min(
    words.length,
    Math.floor(interpolate(frame, [0, Math.min(30, beat.durationFrames * 0.4)], [1, words.length], {
  ...
  return (
    ...
    >
      ...
      >
        {words.map((word, idx) => {
          const isEmphasis = beat.emphasisWords?.some((ew) =>
